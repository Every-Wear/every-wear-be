...
urls=("http://localhost/api/ping" "https://everywear.kro.kr/")
for url in "${urls[@]}"; do
    ...
    if [[ $response == "200" || $response == "301" ]]; then
        ...
        --data '{
            "text":"<@U034N5TBUSZ> '$url' 의 서버 상태가 정상이 아닙니다!! \n sudo ssh -i ~/.ssh/every-wear.key ubuntu@146.56.119.132 / fe:158.180.92.143, <'$url'|여기를 클릭>",
            "channel": "C034R2SQRDY"
        }'
    fi
done